...
class Trainer:
    ...
    def _set_signature_columns_if_needed(self):
        if self._signature_columns is None:
            ...
            # 修改了这里
            self._signature_columns += list(set(["label", "label_ids", "task_ids"] + self.label_names))
    ...
    # def compute_hybrid_loss(self, p, q, loss_1, loss_2):
    ...
    #     reg_alpha = 0.02
    #     hybrid_loss = ce_loss + reg_alpha * kl_loss
    #     return hybrid_loss
    #
    ...
    #         # print("self.args.past_index >= 0")
    #         self._past = outputs_1[self.args.past_index]
    #
    #     if labels is not None:
    #         if unwrap_model(model)._get_name() in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
    #             loss_1 = self.label_smoother(outputs_1, labels, shift_labels=True)
    #             # loss_2 = self.label_smoother(outputs_2, labels, shift_labels=True)
    #             # loss = self.compute_hybrid_loss(outputs_1["logits"], outputs_2["logits"], loss_1, loss_2)
    #         else:
    #             loss_1 = self.label_smoother(outputs_1, labels)
    #             # loss_2 = self.label_smoother(outputs_2, labels)
    #             # loss = self.compute_hybrid_loss(outputs_1["logits"], outputs_2["logits"], loss_1, loss_2)
    #     else:
    #         if isinstance(outputs_1, dict) and "loss" not in outputs_1:
    #             raise ValueError(
    #                 "The model did not return a loss from the inputs, only the following keys: "
    #                 f"{','.join(outputs_1.keys())}. For reference, the inputs it received are {','.join(inputs.keys())}."
    #             )
    #         # We don't use .loss here since the model may return tuples instead of ModelOutput.
    #         loss_1 = outputs_1["loss"] if isinstance(outputs_1, dict) else outputs_1[0]
    ...
    #     # pdb.set_trace()
    #
    #     # return_outputs = False -- in training
    #     return (loss_1, outputs_1) if return_outputs else loss_1
    ...
    def prediction_step(
        ...
        """
        # rank0_print("inputs", inputs)
        # rank0_print("prediction_loss_only", prediction_loss_only)
        # rank0_print("ignore_keys", ignore_keys)

        has_labels = False if len(self.label_names) == 0 else all(inputs.get(k) is not None for k in self.label_names)
        # rank0_print("self.label_names", self.label_names)

        # for attr in dir(self):
        #     rank0_print(attr, getattr(self, attr), "\n")
        # assert False
        # rank0_print("has_labels", has_labels)
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                # rank0_print("is_sagemaker_mp_enabled".center(80, ">"))
                raw_outputs = smp_forward_only(model, inputs)
                if has_labels or loss_without_labels:
                    # rank0_print("has_labels or loss_without_labels".center(90, "/"))
                    ...
                    # rank0_print("not has_labels or loss_without_labels".center(90, "\\"))
                    loss = None
                    if isinstance(raw_outputs, dict):
                ...
                # rank0_print("not is_sagemaker_mp_enabled".center(80, ">"))
                if has_labels or loss_without_labels:
                    # rank0_print("has_labels or loss_without_labels".center(90, "/"))
                    with self.compute_loss_context_manager():
        ...
        # rank0_print("logits", logits)
        # rank0_print("labels_2", labels)
        # rank0_print("\n"*3)
