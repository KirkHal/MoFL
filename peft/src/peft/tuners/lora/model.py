...
from .layer import Conv2d, Embedding, Linear, LoraLayer, MoELinear
...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        if isinstance(target, LoraLayer) and isinstance(target, torch.nn.Conv2d):
            ...
            # print(adapter_name)
            new_module = self._create_new_module(lora_config, adapter_name, target, **kwargs)   # 返回nn.Module，例如Linear
    ...
    def _unload_and_optionally_merge(self, merge=True, progressbar: bool = False):
        ...
        key_list = [key for key, _ in self.model.named_modules() if "lora" not in key]  # 所有非lora的子层
...
class MoELoraModel(LoraModel, nn.Module):

    def __init__(self, model, config, expert_nums) -> None:
        ...
        if not hasattr(self, "peft_config"):
            self.peft_config = {"expert-0": peft_config} if isinstance(peft_config, PeftConfig) else peft_config
        else:
        ...
        self.active_adapter = ["expert-0"]

        # transformers models have a .config attribute, whose presence is assumed later on
        if not hasattr(self, "config"):
            self.config = {"model_type": "custom"}
        for i in range(0, expert_nums):
            adapter_name = f"expert-{i}"
            
            self.peft_config[adapter_name] = self.peft_config["expert-0"]
            self.inject_adapter(self.model, adapter_name, expert_nums=expert_nums)
            if adapter_name != "expert-0":
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str, expert_nums):
        r"""
        ...
        for key in key_list:
            ...
            self._create_and_replace(peft_config, adapter_name, target, target_name, parent, expert_nums=expert_nums, **optional_kwargs)
    ...
    @staticmethod
    def _create_new_module(lora_config, adapter_name, target, expert_nums, **kwargs):
        gptq_quantization_config = kwargs.get("gptq_quantization_config", None)
        AutoGPTQQuantLinear = get_auto_gptq_quant_linear(gptq_quantization_config)
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MoELinear4bit(adapter_name, target.in_features, target.out_features, bias=bias, expert_nums=expert_nums, **fourbit_kwargs)
        elif AutoGPTQQuantLinear is not None and isinstance(target, AutoGPTQQuantLinear):
            new_module = QuantLinear(adapter_name, target, **kwargs)
            target.weight = target.qweight
        elif isinstance(target, torch.nn.Embedding):
            ...
            # print(adapter_name,in_features, out_features, kwargs)
            # print("adding MoELinear layer")
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        if isinstance(target, LoraLayer) and isinstance(target, torch.nn.Conv2d):
            ...
            new_module = self._create_new_module(lora_config, adapter_name, target, expert_nums=expert_nums, **kwargs)   # 返回nn.Module，例如Linear
            if adapter_name != self.active_adapter:
                new_module.requires_grad_(False)
            self._replace_module(parent, target_name, new_module, target)
...
class MoELoraModelV2(LoraModel, nn.Module):
    def __init__(self, model, config, expert_nums) -> None:
        nn.Module.__init__(self)
        self.model = model
    ...
    def _create_new_module(lora_config, adapter_name, target, expert_nums, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MoELinear4bitV2(adapter_name, target.in_features, target.out_features, bias=bias, expert_nums=expert_nums, **fourbit_kwargs)
        elif AutoGPTQQuantLinear is not None and isinstance(target, AutoGPTQQuantLinear):
            new_module = QuantLinear(adapter_name, target, **kwargs)
            target.weight = target.qweight
        elif isinstance(target, torch.nn.Embedding):
...
class MoELoraModelV3(LoraModel, nn.Module):
    def __init__(self, model, config, expert_nums) -> None:
        nn.Module.__init__(self)
        self.model = model
    ...
    def _create_new_module(lora_config, adapter_name, target, expert_nums, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MoELinear4bitV3(adapter_name, target.in_features, target.out_features, bias=bias, expert_nums=expert_nums, **fourbit_kwargs)
        elif AutoGPTQQuantLinear is not None and isinstance(target, AutoGPTQQuantLinear):
            new_module = QuantLinear(adapter_name, target, **kwargs)
            target.weight = target.qweight
        elif isinstance(target, torch.nn.Embedding):
...
class MoELoraModelV4(LoraModel, nn.Module):
    def __init__(self, model, config, expert_nums) -> None:
        nn.Module.__init__(self)
        self.model = model
    ...
    def _create_new_module(lora_config, adapter_name, target, expert_nums, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MoELinear4bitV4(adapter_name, target.in_features, target.out_features, bias=bias, expert_nums=expert_nums, **fourbit_kwargs)
        elif AutoGPTQQuantLinear is not None and isinstance(target, AutoGPTQQuantLinear):
            new_module = QuantLinear(adapter_name, target, **kwargs)
            target.weight = target.qweight
        elif isinstance(target, torch.nn.Embedding):
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        pattern_keys = list(chain(lora_config.rank_pattern.keys(), lora_config.alpha_pattern.keys()))
        target_name_key = next(filter(lambda key: re.match(f".*\.{key}$", current_key), pattern_keys), target_name)
        # print("Adding PEFT module on layer:", target_name_key)
...
class MoELoraModelV5(LoraModel, nn.Module):
    def __init__(self, model, config, expert_nums) -> None:
        nn.Module.__init__(self)
        self.model = model
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str, expert_nums):
        ...
        for key in key_list:
            ...
            self._create_and_replace(peft_config, adapter_name, target, target_name, parent, expert_nums=expert_nums,
                                     **optional_kwargs)
